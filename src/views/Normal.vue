...
export default {
  ...
  methods: {
    ...
    suivant(){
        if(this.numéro === 5){
            ...
            let étoiles = localStorage.getItem("étoiles_normal_courrante");
            localStorage.setItem("étoiles_normal_courrante", Math.floor(this.score/100).toString());
            localStorage.setItem("étoiles_normal", Math.max(Math.floor(this.score/100), étoiles));
            étoiles = localStorage.getItem("étoiles_normal_courrante");
            
            this.étoiles_normal = Number(étoiles);
  ...
  try {
    document.querySelectorAll("audio").forEach(e=>{
      ...
      audio.src = "/public/audio/fondNormal.mp3";
      audio.autoplay = true;
      audio.loop = true;
      const body = document.querySelector("body");
      body.appendChild(audio);

    const response = await fetch('/public/json/normal.json');
